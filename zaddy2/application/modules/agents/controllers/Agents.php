  ...
  if(!empty($_FILES['kyc']['tmp_name']) || !empty($_FILES['photo']['tmp_name'])){
	...
	public function getAgentHistory($agentNo){
		...
		for($i=0;$i<count($history); $i++){
			   ...
			   $itemName =  $history[$i]->itemName;

                $tranData =json_decode($history[$i]->requestObject);

              if(strpos($history[$i]->requestRef,"COMS") !==false){
                  $itemName ="REDEEMED COMMISSION";
              	}
                else if(!empty($tranData->itemName)){
                   $itemName=$tranData->itemName;
                }
                else if($history[$i]->paymentCode=="SHARE"){ 
                	$itemName="FLOAT SHARE"; 
               }
               else if($history[$i]->paymentCode=="LOAD"){ 
                	$itemName="FLOAT LOAD"; 
               } 

               if(!empty($itemName))
                 $history[$i]->itemName =$itemName;
