<?php
	   ...
	   $config['base_url'] = 'http://localhost:81/ellyapp/';

       $conf =file_get_contents($config['base_url'].'conf.json');
       $ENV = json_decode($conf);
       
       define('AFRICELL_API_URL',$ENV->AFRICELL_API_URL);
       define('AFRICELL_DATA_URL',$ENV->AFRICELL_DATA_URL);
       
       define('AFRICELL_USER',$ENV->AFRICELL_USER);
       define('AFRICELL_PASS',$ENV->AFRICELL_PASS);
       define('AFRICELL_VENDORNO',$ENV->AFRICELL_VENDORNO);
       
       define("MTN_AT_URL",$ENV->MTN_AT_URL);
       define("MTN_AUTH_KEY",$ENV->MTN_AUTH_KEY);
       define("MTN_VENDORNO",$ENV->MTN_VENDORNO);
       define("MTN_DATA_NO",$ENV->MTN_DATA_NO);
       define("MTN_KEY",$ENV->MTN_KEY);
        ...
        define("COMMISSION",0.7);
        define("COMMISSION_DAYS",3); //commisison to be redemed for these days back
        
        define("CURRENT_ISSUE","Sorry,The requested service is currently unavailable, please try later.");
		...
		define("FUNDS_RESPONSE_MSG","No funds for provider, contact support");
		...
		//LOGS
		define('LOG_FILE', 'wallet_logs.txt');
		...
		define("REF_PREFIX","ELI");
		define("EXEC_TIMEOUT", 6000);
		define("PROCESS_TIMEOUT", 400);

		//LIVE ISW
		define("MY_TERMINAL",$ENV->MY_TERMINAL);
		define("DEVICE_TERMINAL",$ENV->DEVICE_TERMINAL);
		define("APPSERVICE_URL",$ENV->APPSERVICE_URL);
		define("SVA_BASE_URL",$ENV->SVA_BASE_URL);
		define("QT_BASE_URL",$ENV->QT_BASE_URL);
